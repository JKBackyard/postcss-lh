module.exports = (css, opts) => {
  ...
  // TODO: Maybe this should be a recursive function instead, so we don't need to store
  // the line heights in an array and then return just the last one
  const lineHeight = [];

  css.walkDecls((decl) => {
    // Check:
    // if the declaration is not inside the a print media query
    // if is the root selector
    // and if the property is either font or line-height
